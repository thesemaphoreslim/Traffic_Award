                ...
                do
                {
                    ...
                    switch (mySelection)
                    {
                        case "checkeligibility":
                            ...
                            burstaddress = await Utilities.GetBurstAccountID(burstAddressAPI, yourburstaddress);
                            if (burstaddress.Item1)
                            {
                                if (rafflemembers.Contains(burstaddress.Item2))
                                {
                                    Console.WriteLine("Congratulations, you are eligible for the raffle and have been entered " + rafflemembers.FindAll(item => item == burstaddress.Item2).Count + " time(s)!");
                                    break;
                                }
                                else
                                {
                                    Console.WriteLine("Sorry, you are not eligible for the raffle...");
                                    Console.WriteLine("Send Burst to friends, make purchases from the marketplace," + Environment.NewLine + "or donate to our marketing/development efforts to become eligible!" + Environment.NewLine + Environment.NewLine);
                                    break;
                                }
                            }
                            else
                            {
                                Console.WriteLine("Sorry, you are not eligible for the raffle. We could not find the account ID associated with this BURST address.");
                            }
                            #endregion
                            break;
                        case "runraffle":
